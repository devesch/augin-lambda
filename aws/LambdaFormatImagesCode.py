...
def download_image_from_s3(bucket, key, key_format):
    try:
        s3_resource.Bucket(bucket).download_file(key, save_path)
        return True
    except:
        print("Error during download from s3")
        return False


def upload_image_to_s3(bucket, key_without_format, key_format):
    final_key_name = key_without_format + "." + key_format
    try:
        response = s3_client.upload_file(save_path, bucket, final_key_name)
    except:
        print("Error during upload_file.")
