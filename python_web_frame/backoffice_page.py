...
class BackofficePage(BasePage):
    ...
    def list_html_backoffice_recurrence_failures_table_rows(self, recurrence_failures):
        ...
        if recurrence_failures:
            for recurrence_failure in recurrence_failures:
                html = ReadWrite().read_html("backoffice_recurrence_failures/_codes/html_backoffice_recurrence_failures_table_rows")
                html.esc("recurrence_failure_user_id_val", recurrence_failure["recurrence_failure_user_id"])
                html.esc("recurrence_failure_order_id_val", recurrence_failure["recurrence_failure_order_id"])
    ...
    def list_html_backoffice_models_table_rows(self, models):
        ...
        if models:
            for model in models:
                ...
                html.esc("model_xml_started_val", Date().format_unixtime_to_time(model["model_xml_started"]))
                html.esc("model_xml_memory_usage_val", model["model_xml_memory_usage"])
                html.esc("model_xml_completed_val", Date().format_unixtime_to_time(model["model_xml_completed"]))
                html.esc("model_xml_total_time_val", "{:.2f}".format(float(model["model_xml_total_time"])))
                html.esc("model_xml_to_dynamo_start_val", Date().format_unixtime_to_time(model["model_xml_to_dynamo_start"]))
                html.esc("model_xml_to_dynamo_completed_val", Date().format_unixtime_to_time(model["model_xml_to_dynamo_completed"]))
                html.esc("model_xml_to_dynamo_total_time_val", "{:.2f}".format(float(model["model_xml_to_dynamo_total_time"])))
                html.esc("model_aug_started_val", Date().format_unixtime_to_time(model["model_aug_started"]))
                html.esc("model_aug_percent_val", model["model_aug_percent"])
                html.esc("model_aug_memory_usage_val", model["model_aug_memory_usage"])
                html.esc("model_aug_completed_val", Date().format_unixtime_to_time(model["model_aug_completed"]))
                html.esc("model_aug_total_time_val", "{:.2f}".format(float(model["model_aug_total_time"])))
                html.esc("model_aug_sd_started_val", Date().format_unixtime_to_time(model["model_aug_sd_started"]))
                ...
                html.esc("model_aug_sd_completed_val", Date().format_unixtime_to_time(model["model_aug_sd_completed"]))
                html.esc("model_aug_sd_total_time_val", "{:.2f}".format(float(model["model_aug_sd_total_time"])))
                html.esc("model_processing_total_time_val", "{:.2f}".format(float(model["model_processing_total_time"])))
    ...
    def list_html_backoffice_coupons_table_rows(self, coupons):
        ...
        if coupons:
            for coupon in coupons:
                ...
                html.esc("coupon_brl_discount_val", StrFormat().format_to_money(coupon["coupon_brl_discount"], "brl"))
                html.esc("coupon_usd_discount_val", StrFormat().format_to_money(coupon["coupon_usd_discount"], "usd"))
    ...
    def list_html_backoffice_plans_table_rows(self, plans):
        ...
        if plans:
            for plan in plans:
                ...
                html.esc("plan_price_annually_brl_val", StrFormat().format_to_money(plan["plan_price_annually_brl"], "brl"))
                html.esc("plan_price_annually_usd_val", StrFormat().format_to_money(plan["plan_price_annually_usd"], "usd"))
