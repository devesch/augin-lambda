...
class XMLSigner(XMLSignatureProcessor):
    ...
    def sign(
        ...
        *,
        ...
        signed_info_c14n = self._c14n(signed_info_node, algorithm=self.c14n_alg, inclusive_ns_prefixes=inclusive_ns_prefixes)
        if self.sign_alg.name.startswith("HMAC_"):
            ...
            if self.sign_alg.name.startswith("DSA_"):
                ...
                signature = signing_settings.key.sign(signed_info_c14n, signature_algorithm=ec.ECDSA(algorithm=hash_alg))
    ...
    def _preprocess_reference_uri(self, reference_uris):
        ...
        references = list(ref if isinstance(ref, SignatureReference) else SignatureReference(URI=ref) for ref in reference_uris)
        return references
    ...
    def _build_transforms_for_reference(self, *, transforms_node: _Element, reference: SignatureReference):
        if self.construction_method == SignatureConstructionMethod.enveloped:
            ...
            c14n_xform = SubElement(transforms_node, ds_tag("Transform"), Algorithm=reference.c14n_method.value)  # type: ignore
            if reference.inclusive_ns_prefixes:
                SubElement(c14n_xform, ec_tag("InclusiveNamespaces"), PrefixList=" ".join(reference.inclusive_ns_prefixes))
    ...
    def _build_sig(self, sig_root, references, c14n_inputs, inclusive_ns_prefixes):
        ...
        for i, reference in enumerate(references):
            ...
            payload_c14n = self._c14n(c14n_inputs[i], algorithm=reference.c14n_method, inclusive_ns_prefixes=reference.inclusive_ns_prefixes)
    ...
    def _serialize_key_value(self, key, key_info_node):
        ...
        if self.sign_alg.name.startswith("RSA_") or self.sign_alg.name.startswith("SHA"):
            ...
            named_curve = SubElement(ec_key_value, dsig11_tag("NamedCurve"), URI=self.known_ecdsa_curve_oids[key.curve.name])  # noqa:F841
